...
class QualificationStore {
    ...
    requirements: [
      {
        ...
        description: 'Registrert i Foretaksregisteret, skatteattest, HMS-egenerklæring',
        assessments: {
          bouvet: {
            espdSubmitted: true,
            basis: 'own',
      ...
      },
      {
        ...
        description: 'Kredittvurdering minimum A, ansvarsforsikring min. 10 MNOK',
        assessments: {
          bouvet: {
            espdSubmitted: true,
            basis: 'own',
  ...
  supplierResults = $derived.by(() => {
    const result: Record<
      string,
      { qualified: boolean; met: number; notMet: number; total: number; allAssessed: boolean }
    > = {};
  ...
  updateSupportEntity(
    ...
    supplierId: string,
    entityId: string,
    patch: Partial<Omit<SupportEntity, 'id'>>
  ) {
    const entity = this.getAssessment(reqId, supplierId)?.supportEntities.find(
      (e) => e.id === entityId
    );
