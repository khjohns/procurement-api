...
class SensitivityStore {
	...
	simulatedSum = $derived(
		this.simulatedWeights.reduce((s, w) => s + w.weight, 0)
	);

	hasChanges = $derived(
		this.simulatedWeights.some((w) => w.weight !== w.original)
	);
	...
	setWeight(criterionId: string, weight: number) {
		this.#overrides = { ...this.#overrides, [criterionId]: Math.max(0, Math.min(100, Math.round(weight))) };
	}
