<script lang="ts">
  ...
  let supplier = $derived(supplierIndex >= 0 ? qualification.data.suppliers[supplierIndex] : null);

  let assessment = $derived(requirement && supplier ? requirement.assessments[supplier.id] : null);
  ...
  <div class="panel">
    ...
    <div class="supplier-nav">
      ...
      <button
        ...
        disabled={supplierIndex >= qualification.data.suppliers.length - 1}>›</button
      >
    </div>
    ...
    <div class="progress-strip">
      ...
      <div class="progress-bar">
        <div
          class="progress-fill"
          style="width: {noteProgress.total > 0
            ? (noteProgress.filled / noteProgress.total) * 100
            : 0}%"
        ></div>
      </div>
    </div>
        ...
        <div class="summary-header">
          ...
          {#if assessment}
            <span
              ...
              class:verdict-met={assessment.verdict === 'met'}
              class:verdict-not-met={assessment.verdict === 'not_met'}
            >
              {#if assessment.verdict === 'met'}Oppfylt{:else if assessment.verdict === 'not_met'}Ikke
                oppfylt{:else}Ikke vurdert{/if}
            </span>
          {/if}
        </div>
