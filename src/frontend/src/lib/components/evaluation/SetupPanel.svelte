<script lang="ts">
	...
	let contractValueDisplay = $derived(
		isEditingValue ? editValue : (evaluation.data.contractValue > 0 ? fmt.format(evaluation.data.contractValue) : '')
	);
	...
	let validationItems = $derived.by(() => {
		...
		if (evaluation.activeMethod === 'pris' && evaluation.data.suppliers.some((s) => !s.price)) items.push('Fyll inn tilbudspriser');
		return items;
	});
</script>
