<script lang="ts">
	...
	function importProcurement(proc: MockProcurement) {
		...
		// Add criteria via store methods (IDs generated by store)
		for (const c of proc.criteria) {
			const type = c.type === 'price' ? 'price' as const : 'quality' as const;
			const criterionId = evaluation.addCriterion(c.name, type);
			// addCriterion creates one default sub-criterion; update its weight
			const criterion = evaluation.data.criteria.find((cr) => cr.id === criterionId);
			if (criterion?.subcriteria[0]) {
				evaluation.setSubCriterionWeight(criterion.subcriteria[0].id, c.weight);
			}
		}

		// Add suppliers via store methods
		for (const name of proc.suppliers) {
			evaluation.addSupplier(name);
		}
