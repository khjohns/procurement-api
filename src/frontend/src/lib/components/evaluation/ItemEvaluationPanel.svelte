<script lang="ts">
  ...
  let bestItemAvg = $derived(
    items.length > 0 ? Math.max(...items.map((item) => itemScore(item, itemCriteria))) : 0
  );
        ...
        <div class="agg-options">
          {#each aggregationOptions as opt}
            <button
              ...
              onclick={() => evaluation.setAggregation(subCriterionId, opt.id)}
            >
              <span class="agg-radio" class:checked={aggregation === opt.id}></span>
              {opt.label}
            </button>
          {/each}
        </div>
            ...
            {#each items as item (item.id)}
              ...
              <tr class="item-row">
                ...
                {#each itemCriteria as ic}
                  ...
                  {@const isBest = score === columnBests[ic.id] && score > 0}
                  <ItemScoreCell
                    {score}
                    {isBest}
                    onchange={(v) =>
                      evaluation.setItemScore(subCriterionId, supplierId, item.id, ic.id, v)}
                  />
                {/each}
                <td class="cell-avg">
                  <span class="avg-value tier-{avgTier}" class:avg-best={isItemBest}>
                    {avg.toFixed(1)}
                  </span>
                </td>
              </tr>
          ...
          <tfoot>
            <tr class="item-total-row">
              ...
              <td class="cell-total-final">
                <span class="total-final-value tier-{scoreTier(aggregatedScore)}">
