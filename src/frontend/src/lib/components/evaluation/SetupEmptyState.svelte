<script lang="ts">
	...
	function addSubCriterion(criterionId: string) {
		...
		requestAnimationFrame(() => {
			const el = document.querySelector(`[data-sub-id="${id}"] .sub-name-input`) as HTMLInputElement;
			el?.focus();
		});
	}
...
<div class="setup-main">
	{#if evaluation.data.criteria.length === 0}
		<div class="empty-state">
			...
			<div class="empty-desc">Importer kriterier fra Doffin, eller legg til manuelt i panelet til høyre.</div>
			<button class="empty-action" onclick={addCriterion}>
				+ Legg til første kriterium
			</button>
		</div>
	{:else}
		...
		<div class="criteria-editor">
					...
					<div class="criterion-row criterion-row-group">
						...
						<div class="criterion-name">
							<input
								...
								oninput={(e) => evaluation.renameCriterion(criterion.id, (e.target as HTMLInputElement).value)}
						...
						<button
							...
							onclick={() => evaluation.setCriterionType(criterion.id, criterion.type === 'quality' ? 'price' : 'quality')}
						>
						...
						<button class="criterion-remove" onclick={() => evaluation.removeCriterion(criterion.id)} title="Fjern kriterium">×</button>
					</div>
						...
						<div class="criterion-row criterion-row-sub" data-sub-id={sub.id}>
							<div class="criterion-weight">
								<input
									...
									max="100"
									value={sub.weight}
									oninput={(e) => evaluation.setSubCriterionWeight(sub.id, parseInt((e.target as HTMLInputElement).value) || 0)}
								/>
			...
			<!-- Total row -->
			<div class="criteria-total" class:total-valid={evaluation.totalWeight === 100} class:total-invalid={evaluation.totalWeight > 0 && evaluation.totalWeight !== 100}>
				<div class="criterion-weight">
					<span class="weight-value weight-value-total">{evaluation.totalWeight}<span class="weight-pct">%</span></span>
					<div class="weight-bar weight-bar-total">
						<div class="weight-bar-fill weight-bar-fill-total" style="width: {Math.min(100, evaluation.totalWeight)}%"></div>
					</div>
				</div>
				<div class="total-label">Total vekting</div>
				{#if evaluation.totalWeight !== 100 && evaluation.totalWeight > 0}
					<span class="total-warning">{evaluation.totalWeight < 100 ? `${100 - evaluation.totalWeight}% gjenstår` : `${evaluation.totalWeight - 100}% for mye`}</span>
				{/if}
			</div>
		</div>
	{/if}
