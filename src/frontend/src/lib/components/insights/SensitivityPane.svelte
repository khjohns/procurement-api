	...
	<div class="section">
						...
						{#if tp.type === 'score'}
							<div class="tp-line">
								...
								<span class="tp-value {tierClass(Math.abs(tp.delta))}">
									{tp.delta >= 0 ? `\u2212${tp.delta.toFixed(1)}` : `+${Math.abs(tp.delta).toFixed(1)}`} poeng margin
								</span>
							</div>
						{:else}
		...
		{#if sensitivity.pairAnalyses.length > 0}
			<div class="robustness-summary">
				Resultatet er <strong>{sensitivity.robustnessLabel}</strong>.
				Vekten p&aring; {sensitivity.mostSensitive.criterionName} ({sensitivity.mostSensitive.weight} %)
				er den mest sensitive parameteren.
			</div>
		{/if}
	</div>
	...
	<div class="section">
		...
		{#if sensitivity.winnerChanged}
			<div class="ranking-alert">
				Rangeringen endres! {sensitivity.simulatedRanking[0]?.name} g&aring;r forbi {evaluation.ranking[0]?.supplier.name}.
			</div>
		{:else if sensitivity.rankingChanged}
			<div class="ranking-alert ranking-alert-minor">
				Rangeringen endres (samme vinner).
			</div>
		{/if}
...
<style>
	...
	.tier-stable { color: var(--color-score-high); }
	.tier-moderate { color: var(--color-vekt); }
	.tier-vulnerable { color: var(--color-score-low); }
	...
	@media (max-width: 1024px) {
		.ranking-comparison { grid-template-columns: 1fr; }
	}
</style>
