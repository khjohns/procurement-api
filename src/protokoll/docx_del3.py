...
def _procedure(doc, procurement, activities, eforms=None):
    ...
    docx_info_table(
        ...
        [
            ...
            (
                "Begrunnelse for forhandling uten kunngjøring / uten konkurranse:",
                no_pub_str,
            ),
            (
                "Begrunnelse for ikke å dele opp i delkontrakter (FOA \u00a7 19-4):",
                None,
            ),
...
def _formal_rejection(doc, activities, org_lookup):
    ...
    add_manual(
        p,
        "[Avgjør hvilke avvisninger som gjelder formalfeil (\u00a7 24-1) vs. kvalifikasjon (\u00a7 24-2)]",
    )
    ...
    docx_add_table_with_manual(
        doc, ["Leverandørens navn", "Begrunnelsen for avvisningen", "Dato sendt"], rows
    )


def _preliminary_qualification(doc, procedure):
    doc.add_heading(
        "Foreløpig kvalifikasjonsvurdering, jf. FOA \u00a7 17-1 annet ledd", level=3
    )
...
def _qualification(doc, eforms=None):
    ...
    add_manual(
        p,
        "[Kvalifikasjonskrav og -vurdering er ikke tilgjengelig via API. Fyll inn basert på konkurransegrunnlaget.]",
    )
...
def _supplier_selection(doc, procedure, activities):
    ...
    if procedure == "Open":
        doc.add_paragraph(
            "Ikke relevant (åpen anbudskonkurranse — ingen utvelgelsesfase)."
        )
    elif procedure in (
        "Limited",
        ...
        if qualifying:
            ...
            docx_add_table_with_manual(
                doc,
                ["Leverandørens navn", "Begrunnelse for utvelgelse"],
                [(None, None)],
            )
    else:
...
def _negotiations(doc, procedure):
    ...
    if procedure in ("Competitive negotiated", "Innovation partnership"):
        ...
        label = (
            PROCEDURE_MAP.get(procedure, procedure)
            if procedure != "Open"
            else "åpen anbudskonkurranse"
        )
...
def _dialog(doc, procedure):
    ...
    if procedure == "Competitive dialogue":
        doc.add_paragraph(
            "Konkurransepreget dialog \u2014 dialog er en sentral del av prosedyren."
        )
    p = doc.add_paragraph()
    add_checkbox(p, "Det ble ikke gjennomf\u00f8rt dialog")
    add_manual(p, " [Bekreft]")
    docx_add_table_with_manual(
        doc, ["Leverand\u00f8rens navn", "Dato for dialog"], [(None, None)]
    )
...
def _bids_in_evaluation(doc, activities, org_lookup):
    ...
    if evaluated:
        ...
        rows = [("", None)]
    docx_add_table_with_manual(
        doc, ["Tilbuds-/løpenummer", "Leverandørenes navn"], rows
    )
...
def _award_criteria(doc, eforms):
    ...
    if not eforms:
        add_manual(
            doc.add_paragraph(),
            "[Fyll inn tildelingskriterier fra konkurransegrunnlaget]",
        )
        return
    ...
    if not criteria:
        add_manual(
            doc.add_paragraph(),
            "[Ingen tildelingskriterier funnet i kunngjøringen — fyll inn manuelt]",
        )
        return
    ...
    if max_participants is not None and max_participants == 1:
        ...
        rows = [
            ...
            (
                ...
                f"Ja (maks {max_participants} deltakere)",
            ),
...
def _market_dialogue_and_conflicts(doc):
    ...
    doc.add_heading(
        "Dialog med markedet f\u00f8r konkurranse, jf. FOA kap. 12", level=4
    )
    ...
    docx_info_table(
        ...
        [
            ...
            (
                "Leverand\u00f8r(er) som deltok i dialog f\u00f8r konkurransen (jf. FOA \u00a7 12-2):",
                None,
            ),
